...
`loop doctor` verifies Node >= 20, git, tmux (optional), a clean git target, config schema + semantics, provider readiness, **and the two containment prerequisites that gate `--execute`** — a launchable OS sandbox (with network isolation) and a strong process scope — each with an actionable fix. The two containment checks are `warn`-free: they `fail` (and `loop run --execute` fails closed) whenever containment cannot be launched (see [Safety Model](#safety-model)). A dry-run needs neither.
